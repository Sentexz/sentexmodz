--[[
    ...
    Banner con reintentos y tiempo de espera (URL: i.ibb.co).
]]
...
-- ========== ACCIONES ORIGINALES ==========
-- (Incluye todas las funciones: curar, revivir, vehículos, noclip, freecam, etc.)
-- Por brevedad, no pego aquí las 1000 líneas, pero asegúrate de que tu script las tenga.
-- Si necesitas el script completo con todas las acciones, dímelo y te lo paso en un solo bloque.

-- ========== PARTE GRÁFICA CON BANNER Y REINTENTOS ==========
...
local BANNER_LOAD_ATTEMPTS = 0
local MAX_ATTEMPTS = 5
local BANNER_URL = "https://i.ibb.co/LD0xLqv1/434e648a-51dd-4457-9900-bc6321d62a13.png"

-- Función mejorada con reintentos y verificación
local function LoadBannerFromURL()
    Citizen.CreateThread(function()
        while BANNER_LOAD_ATTEMPTS < MAX_ATTEMPTS and not CUSTOM_BANNER_LOADED do
            BANNER_LOAD_ATTEMPTS = BANNER_LOAD_ATTEMPTS + 1
            print("[SENTEX] Intento " .. BANNER_LOAD_ATTEMPTS .. " de " .. MAX_ATTEMPTS .. " para cargar banner")
            ...
            -- Esperamos 3 segundos para que el DUI intente cargar
            _w(3000)
            ...
            if duiHandle and duiHandle ~= 0 then
                ...
                if texture then
                    ...
                    print("[SENTEX] Banner cargado exitosamente en el intento " .. BANNER_LOAD_ATTEMPTS)
                    return
                end
            end
            
            print("[SENTEX] Intento " .. BANNER_LOAD_ATTEMPTS .. " fallido. Reintentando en 2 segundos...")
            _w(2000) -- Espera antes del siguiente intento
        end
        
        if not CUSTOM_BANNER_LOADED then
            print("[SENTEX] No se pudo cargar el banner después de " .. MAX_ATTEMPTS .. " intentos. Usando gradiente.")
...
Citizen.CreateThread(function()
    ...
    LoadBannerFromURL()  -- Lanzamos el proceso de carga con reintentos
    _w(1000)
...
-- El resto del código (noclip, freecam, etc.) sigue igual...
-- (Aquí irían las funciones que ya tenías)
